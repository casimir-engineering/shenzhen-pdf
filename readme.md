...
- **Exports exactly what you see** — Save as PDF, Print, Copy Page, and Copy Page Image reproduce the on-screen Markdown pages: same margins, pagination, text size, and reading theme. Markdown's colors are ours to choose, so exporting the dark ones is exporting what you asked for; a PDF's colors are its own content, which is why exporting *those* always uses the original. Adjustable text size (<kbd>A−</kbd> / <kbd>A＋</kbd>) is remembered across documents.
