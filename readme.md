...
<sub>Latest <b>26.8.31-1</b> · Apple Silicon</sub>
...
- **Shenzhen Files as your file manager** <sub>macOS</sub> — Install Shenzhen Files and it becomes the app's file manager on its own, with nothing to configure; *Settings ▸ File Manager* switches back to Finder whenever you want, and an explicit choice always wins (the automatically-derived one is marked *(Automatic)* so you can tell them apart). It handles browsing in both directions: **Open…** (<kbd>Cmd+O</kbd>) and the path prompt (<kbd>Cmd+Shift+O</kbd>) open it at the folder you're most likely to want — the current document's, else your last opened one's, else home — and *Show in Folder* reveals there. Picking a document in it opens the document in ShenzhenPDF whenever ShenzhenPDF is the system handler for that type, which the one-click default-reader button below sets up. Every launch falls back to Finder if Shenzhen Files is missing or fails to start. **Save panels stay native by design** — saving must hand a chosen destination back to the app, and no external file manager can return one, so Save as PDF and every export keep the standard macOS panel.
