@echo off
rem Build and run portable/win/tests/search_differential.c natively.
rem
...
rem in glib_shim_search and LAYERS on the shared glib_shim, so both directories
rem are on the path and glib_shim_search must come FIRST -- it is the one that
...
if not exist "%REPO%\portable\core\shenzhen_pdf_core.h" (
  echo [search-differential] "%REPO%" does not look like the ShenzhenPDF repo
  exit /b 93
)
...
if not exist "%SPDF_VCVARS%" (
  echo [search-differential] no vcvarsall.bat at "%SPDF_VCVARS%"; set SPDF_VCVARS
  exit /b 91
)
...
if errorlevel 1 (
  echo [search-differential] vcvarsall.bat %SPDF_ARCH% failed
  exit /b 92
)
:have_cl
...
cl.exe /nologo /W3 /O2 /fp:precise /MD /TC /D_CRT_SECURE_NO_WARNINGS /wd4505 ^
  /I"%~dp0glib_shim_search" /I"%~dp0glib_shim" ^
  /I"%REPO%\portable\core" /I"%REPO%\portable\win\src" /I"%REPO%\portable\linux\gtk4" ^
  /Fo"%SPDF_OUT%\search_differential." /Fe"%SPDF_OUT%\search_differential.exe" ^
  "%~dp0search_differential.c"
if errorlevel 1 (
  echo [search-differential] cl.exe failed
  exit /b 65
)

"%SPDF_OUT%\search_differential.exe"
exit /b %ERRORLEVEL%
