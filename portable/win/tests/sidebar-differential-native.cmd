@echo off
rem Build and run portable/win/tests/sidebar_differential.c natively.
rem
rem Exactly the shape of search-differential-native.cmd, with one more layer of
rem glib shim: glib_shim_sidebar layers on glib_shim_search which layers on
rem glib_shim, so all three directories are on the include path and the sidebar
rem one comes FIRST -- it is the one that must resolve as <glib.h>.
rem
...
if not exist "%REPO%\portable\core\shenzhen_pdf_core.h" (
  echo [sidebar-differential] "%REPO%" does not look like the ShenzhenPDF repo
  exit /b 93
)
...
if not exist "%SPDF_VCVARS%" (
  echo [sidebar-differential] no vcvarsall.bat at "%SPDF_VCVARS%"; set SPDF_VCVARS
  exit /b 91
)
...
if errorlevel 1 (
  echo [sidebar-differential] vcvarsall.bat %SPDF_ARCH% failed
  exit /b 92
)
:have_cl
...
rem of them being static. Same flags as the search differential otherwise.
cl.exe /nologo /W3 /O2 /fp:precise /MD /TC /D_CRT_SECURE_NO_WARNINGS /wd4505 ^
  /I"%~dp0glib_shim_sidebar" /I"%~dp0glib_shim_search" /I"%~dp0glib_shim" ^
  /I"%REPO%\portable\core" /I"%REPO%\portable\win\src" /I"%REPO%\portable\linux\gtk4" ^
  /Fo"%SPDF_OUT%\sidebar_differential." /Fe"%SPDF_OUT%\sidebar_differential.exe" ^
  "%~dp0sidebar_differential.c"
if errorlevel 1 (
  echo [sidebar-differential] cl.exe failed
  exit /b 65
)

"%SPDF_OUT%\sidebar_differential.exe"
exit /b %ERRORLEVEL%
