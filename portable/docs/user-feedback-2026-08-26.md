...
|---|---|---|---|---|
| UF-01 | Native text multi-click selection | Double-click over selectable document text selects one word. Triple-click selects the containing text block/paragraph. Plain left-drag selection and link clicking remain unchanged. | Implemented and independently reviewed on macOS | Typed core and real `SPDFDocumentView` event tests cover word/block selection, `down/up/down/up` link cancellation, link-drag range selection, control-click, and interaction cleanup. |
...
| UF-08 | Double-click titlebar to maximize | Double-clicking an actually draggable, empty titlebar/tab-row region invokes the native window zoom/maximize behavior. Double-clicking tabs, buttons, and fields keeps their existing behavior. | Implemented on macOS and Linux | GTK4 empty-header tests in `4c1989be2`; macOS chrome-action policy and full app build pass. |
