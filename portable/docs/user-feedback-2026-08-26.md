...
| UF-11 | Read-only Markdown | Open common `.md` Markdown from Claude/Obsidian; render headings, emphasis, links, block quotes, lists, tables where supported, and fenced code. Code fences use syntax highlighting and expose a supported-language selector when the declared language is absent/unknown. | Foundation independently approved; app integration pending | MD4C parser/model/renderer/resource and language services pass seven focused suites normally and under ASan/UBSan. Native tab/session UI wiring remains a separate slice. |
| UF-12 | Markdown language search, print, and PDF export | Markdown appears in the normal tab/session workflow. The syntax-language selector has a narrowing search field and full keyboard navigation. Print and Save as PDF use an A4 portrait pagination model that fills pages while avoiding a section heading in roughly the final quarter when its following content cannot fit. | Foundation independently approved; app integration pending | Bounded cancellable search, language filtering, TextKit pagination, PDF export, image budgets, descriptor-pinned resources, and smart heading placement are verified; native print-panel/tab integration remains pending. |
