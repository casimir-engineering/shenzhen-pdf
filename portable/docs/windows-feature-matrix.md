...
| Verified daily auto-updater (kept off the launch path, hourly/day-change/wake catch-ups, release highlights, size clamp), Check for Updates…, auto-check setting | 26.6.27-1 / 26.7.17-1 / 26.8.31-1 | ✓ (Developer ID, notarization) | ✓ (minisign, deb + tarball) | **MISSING** | Nothing in `portable/win/src`. Verification core is portable: GTK `spdf_updater.c` (2,369; minisign/ed25519 via OpenSSL, `updater_test.c`), `spdf_update_version.c` (75, TF). Install path is Windows-specific (Authenticode + self-replacing exe or MSIX; plan §2.4). Mac `SPDFUpdater.mm` (1,583), `SPDFUpdaterDownloadBounds.mm` (TF) | Prerequisite: a published Windows binary and a release-asset layout |
| One-click default PDF reader | 26.6.17-1 | ✓ | ✓ | **MISSING** | GTK `spdf_default_reader.c` (263, `default_reader_test.c`), mac `SPDFMacDefaultReader.mm` (152). Windows: `IApplicationAssociationRegistration` / Default Apps deep link; needs a ProgID and file association first (plan §2.4) | |
...
| Keyboard Shortcuts cheat sheet (Help / F1), About | 26.6.17-1 | ✓ | ✓ | **MISSING** | The menu table is the intended data source (`spdf_win_menu.h` header); GTK `spdf_shortcuts.c` generates its sheet from the same kind of table; mac `ShenzhenMacDelegate+ShortcutHelp.mm` | |
...
| App icon, taskbar identity, `.pdf` file association | 26.8.27-1 (new icon) | ✓ | ✓ | **MISSING** | No `.rc`/`.ico`/manifest under `portable/win` (`git ls-files`); the exe carries the default icon; no ProgID | Artwork exists in `portable/mac/Assets.xcassets` |
