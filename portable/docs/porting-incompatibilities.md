...
| Windows synchronization/threading primitives | Cache/render scheduling | First slice renders synchronously through MuPDF; next step should add a small C++ portable worker queue using `std::thread`, `std::mutex`, and cancellation tokens. |
...
- Add a portable render cache and background renderer so large PDFs do not block the UI.
