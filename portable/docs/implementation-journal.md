...
13. First launch should feel instant, and tab hover titles must not stick.
   ...
   - Changed: Mac max-zoom document panning now renders throttled 1x viewport crops during pan/inertia, then refreshes the full backing-scale crop when motion settles; document pan and inertia scrolls explicitly notify the reader so live crop rendering actually runs before inertia ends.
   ...
   - Changed: Mac tab strip now captures the whole tab-strip mouse sequence instead of letting empty titlebar pixels fall through to native window dragging, protecting tab reorder and drag-out gestures.
