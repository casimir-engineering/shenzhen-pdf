...
// fresh Gatekeeper assessment, cold state files, cold or remote (DriveFS)
...
// app's processes by flock inside spdf_state_migrate_dir, and idempotent: a
// hand-created YAML file wins and its JSON sibling is left untouched.
...
- (NSString*)displayVersion {
    ...
    if (version.length == 0) version = @"26.7.17";
...
- (BOOL)validateMenuItem:(NSMenuItem*)menuItem {
    ...
    if (action == @selector(openDocument:) || action == @selector(openPathPrompt:) ||
        ...
        action == @selector(openStateFile:) || action == @selector(revealSettingsFolder:) ||
