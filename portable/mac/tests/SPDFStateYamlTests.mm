...
static void test_migration(void) {
    ...
    // YAML already present -> JSON untouched, YAML wins.
    {
        NSString* dir = make_temp_dir();
        ...
        if (migrated != 0) fail(@"yaml wins: no migration", @"migrated despite yaml present");
        ...
        if (![read_file([dir stringByAppendingPathComponent:@"settings.json"]) isEqualToString:@"{\"fitMode\":1}"])
            fail(@"yaml wins: json untouched", @"json changed");
        [fm removeItemAtPath:dir error:nil];
    }
